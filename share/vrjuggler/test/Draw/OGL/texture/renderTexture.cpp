...
void tex::bind( Texture& texture, int& texObjectID, int mipmapLevelOfDetail, int bordersize )
{
...
void tex::unbind( Texture& texture, int& texObjectID )
{
...
bool tex::isBound( const Texture& texture, const int& texObjectID )
{
   if (texObjectID == -1)
      return false;
...
bool tex::load( const Texture& texture, int mipmapLevelOfDetail, int bordersize )
{
   int format, type;
...
//              better performance
void tex::render( const Texture& texture, const int& texObjectID )
{
   const Image& image = texture.image();
